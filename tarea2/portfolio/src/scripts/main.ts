/* Parallax */

let text = document.getElementById("text") as HTMLElement;
let planet1 = document.getElementById("planet1") as HTMLElement;
let planet2 = document.getElementById("planet2") as HTMLElement;
let planet3 = document.getElementById("planet3") as HTMLElement;
let planet4 = document.getElementById("planet4") as HTMLElement;

window.addEventListener("scroll", () => {
  let value = window.scrollY;

  text.style.marginTop = `${value * 2.5}px`;
  planet1.style.marginTop = `${value * 0.5}px`;
  planet2.style.marginLeft = `${value * 1.0}px`;
  planet3.style.marginLeft = `${value * -1.0}px`;
  planet4.style.marginTop = `${value * -0.5}px`;
});

/* Mobile View */

function toggleMobileMenu(): void {
  const mobileNav = document.querySelector(".mobile-nav-links") as HTMLElement;
  if (mobileNav) {
    mobileNav.classList.toggle("show");
  }
}

function updateImagesForMobile(): void {
  const images = document.querySelectorAll(
    ".planet-image",
  ) as NodeListOf<HTMLImageElement>;

  images.forEach((image, index) => {
    const mobileSrc = `media/planet${index + 1}mobile.png`;
    const desktopSrc = `media/planet${index + 1}.png`;

    if (window.innerWidth <= 768) {
      image.src = mobileSrc;
    } else {
      image.src = desktopSrc;
    }
  });
}

updateImagesForMobile();

window.addEventListener("resize", updateImagesForMobile);
